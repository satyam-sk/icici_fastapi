...
# API A: Fetch customer based on last 4 digits of mobile
...
# API B: Fetch card details based on customer id
@app.get("/api/customers/{customer_id}/cards", response_model=List[Card])
async def get_customer_cards(customer_id: int):
    """
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT cust_id FROM customers WHERE cust_id = %s", (customer_id,))
                if not cur.fetchone():
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
...
# API C: Fetch account details based on customer id
@app.get("/api/customers/{customer_id}/accounts", response_model=List[Account])
...
# API D: Fetch last N transactions based on customer id
@app.post("/api/customers/{customer_id}/transactions/recent", response_model=List[Transaction])
async def get_recent_transactions(customer_id: int, request: TransactionsRequest):
    """
...
# API E: Block a credit card based on customer id and last 4 digits
...
# Additional utility endpoint to get customer by ID
